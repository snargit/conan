        ...
        macro(set_imported_configs target foundConfs)
            ...
            if ("MINSIZEREL" IN_LIST existingConfigs)
                if (NOT "RELEASE" IN_LIST existingConfigs)
                    set_property(TARGET ${target} APPEND PROPERTY MAP_IMPORTED_CONFIG_RELEASE MinSizeRel)
                endif()
                if (NOT "RELWITHDEBINFO" IN_LIST existingConfigs)
                    set_property(TARGET ${target} APPEND PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO MinSizeRel)
                endif()
            endif()
            if ("RELWITHDEBINFO" IN_LIST existingConfigs)
                if (NOT "MINSIZEREL" IN_LIST existingConfigs)
                    set_property(TARGET ${target} APPEND PROPERTY MAP_IMPORTED_CONFIG_MINSIZEREL RelWithDebInfo)
                endif()
                if (NOT "RELEASE" IN_LIST existingConfigs)
                    set_property(TARGET ${target} APPEND PROPERTY MAP_IMPORTED_CONFIG_RELEASE RelWithDebInfo)
                endif()
            endif()
        endmacro()
